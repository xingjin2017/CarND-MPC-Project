 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      AD<double> steer0 = vars[steer_start + i];
      AD<double> throttle0 = vars[throttle_start + i];

      //AD<double> f0 = polyeval(coeffs, x0);
      //AD<double> psides0 = CppAD::atan(polydirevative(coeffs, x0));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  size_t n_vars = num_vars * N + 2 * (N - 1);
  // TODO: Set the number of constraints
  size_t n_constraints = N * num_vars;
  ...
  for (int i = 0; i < solution.x.size(); ++i) {
    solutionx.push_back(solution.x[i]);
  }
